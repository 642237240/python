...
class SplitterExampleFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.minpane = 0
        self.initpos = 0
        self.sp = wx.SplitterWindow(self, style=wx.SP_PERMIT_UNSPLIT)
    ...
    def OnSetMin(self, evt):
        minpane = wx.GetNumberFromUser('Enter the minimum pane size', 
            '', 'Minimum Pane Size', self.minpane, 0, 1000, self)
        if minpane != -1:
    ...
    def OnSetPos(self, evt):
        initpos = wx.GetNumberFromUser(
            "Enter the initial sash positin(to be used in the Split call) ",
