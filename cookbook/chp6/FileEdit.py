...
class FileEditorFrame(wx.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # Attributes
        self.file = None
        self.style = style = wx.TE_MULTILINE | wx.TE_RICH2
    ...
    def OnOpen(self, event):
        '''Handle Open'''
        #self.PushStatusText(str.encode(event.GetId()))
    ...
    def OnSave(self, event):
        '''Handle Save/Save As'''
        evt_id = event.GetId()
        if evt_id in (wx.ID_SAVE, wx.ID_SAVEAS):
            self.PushStatusText(self.file)
            if self.file:
                self.Save(self.file)
            else:
                self.DoSaveAs()
    ...
    def OnExit(self, event):
        ...
        if self.txtctrl.IsModified():
            ...
            result = wx.MessageBox(message, "Save Changes?", style=style)
            if result == wx.YES:
